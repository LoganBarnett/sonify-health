//! Resolve the directory holding the compiled Elm frontend assets and
//! export it so the `rust_embed`-derived `Frontend` type can bake them
//! into the binary.
//!
//! In a Nix release build the packaging passes `SONIFY_HEALTH_FRONTEND_DIR`
...
fn main() -> Result<(), Box<dyn Error>> {
  let frontend_dir = match std::env::var_os("SONIFY_HEALTH_FRONTEND_DIR") {
  ...
  // `rust_embed` reads `$SONIFY_HEALTH_FRONTEND_DIR` at macro-expansion time;
  // re-export the resolved (and defaulted) path so the attribute always sees
  // a concrete directory.
  println!(
    "cargo:rustc-env=SONIFY_HEALTH_FRONTEND_DIR={}",
    frontend_dir.display(),
  );
  println!("cargo:rerun-if-env-changed=SONIFY_HEALTH_FRONTEND_DIR");
