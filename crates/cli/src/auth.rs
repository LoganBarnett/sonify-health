...
//
// Flow:
//   1. `GET /auth/login`    → store csrf+nonce in session, redirect to
//      the provider
//   2. `GET /auth/callback` → verify state, exchange code, store user
//      in session
...
/// `GET /auth/callback` — receive the authorization code from the
/// OIDC provider.
